// Bluarmor Mock Data

export const navigationLinks = [
  { label: 'Products', href: '#products' },
  { label: 'About', href: '/about' },
  { label: 'Support', href: '/support' },
];

export const heroContent = {
  headline: 'BUILT TO LAST',
  subheadline: 'Designed to evolve',
  description: 'Seamless operation. Engineered to disappear. Adaptive mesh technology that gets smarter with every mile, so you can just ride.',
  cta: 'Explore the 2026 Armoury',
};

export const manifestoText = "CONNECTIVITY ISN'T A FEATURE. IT IS INFRASTRUCTURE.";

export const commandCentreFeatures = [
  {
    id: 'eridegrid',
    title: 'eRideGrid',
    description: 'Mesh-over-cloud technology that creates a self-healing network between riders. When one rider loses signal, others bridge the gap.',
  },
  {
    id: 'portweave',
    title: 'PortWeave',
    description: 'Connect up to 6 devices simultaneously. Phone, GPS, music, intercom — all unified under one intelligent system.',
  },
  {
    id: 'audiorange',
    title: 'Audio Range',
    description: 'High-definition audio engineered for wind noise cancellation. Crystal clear communication at any speed.',
  },
  {
    id: 'heyblu',
    title: 'Hey Blu',
    description: 'Hands-free voice commands that understand context. Navigate, call, and control without lifting a finger.',
  },
  {
    id: 'latchon',
    title: 'Latch On Protocol',
    description: 'Bridges base units across different helmet systems. Universal compatibility, zero compromise.',
  },
  {
    id: 'otaupdate',
    title: 'OTA Update',
    description: 'Wireless firmware updates that improve your system over time. Your gear evolves with the technology.',
  },
];

export const products = [
  {
    id: 'blu3-e',
    name: 'BLU3 E',
    tagline: 'Entry-level excellence',
    price: '₹12,999',
    features: ['Mesh Network', 'Voice Commands', '12hr Battery'],
  },
  {
    id: 'blu5-pro',
    name: 'BLU5 Pro',
    tagline: 'Professional grade',
    price: '₹18,999',
    features: ['6-Device Connect', 'HD Audio', 'OTA Updates'],
  },
  {
    id: 'blu7-ultra',
    name: 'BLU7 Ultra',
    tagline: 'Ultimate performance',
    price: '₹24,999',
    features: ['Full eRideGrid', 'Premium Audio', 'All Features'],
  },
];

export const riderStories = [
  {
    id: 1,
    name: 'Arjun Mehta',
    location: 'Ladakh Expedition',
    quote: 'When the network dropped at 15,000 feet, the mesh kept us connected. That is infrastructure.',
    rides: '12,000+ km',
  },
  {
    id: 2,
    name: 'Priya Sharma',
    location: 'Daily Commuter',
    quote: 'It just works. Every day. That\'s all I needed.',
    rides: '8,500+ km',
  },
  {
    id: 3,
    name: 'Vikram Singh',
    location: 'Group Ride Leader',
    quote: 'Managing 15 riders used to be chaos. Now it is seamless.',
    rides: '25,000+ km',
  },
];

export const packUnitedContent = {
  headline: 'THE PACK UNITED. NO MATTER WHAT THEY RIDE.',
  sections: [
    {
      id: 'democratic-mesh',
      title: 'The Democratic Mesh',
      description: 'Every rider is equal on the network. No hierarchy. No single point of failure. True distributed communication.',
    },
    {
      id: 'universal-handshake',
      title: 'The Universal Handshake',
      description: 'Connect with any Bluarmor device instantly. One tap. No pairing drama. No compatibility questions.',
    },
    {
      id: 'digital-pillion',
      title: 'The Digital Pillion',
      description: 'Your passenger connects as seamlessly as you do. Same quality. Same features. Same experience.',
    },
  ],
  cta: 'Build Your Pack',
};

export const whyBluarmor = [
  {
    id: 'innovation',
    title: 'INNOVATION',
    description: 'Engineering solutions that don\'t exist yet. We build the infrastructure others will follow.',
  },
  {
    id: 'quality',
    title: 'QUALITY',
    description: 'Every component tested beyond limits. Every connection verified. Every unit accountable.',
  },
  {
    id: 'ruggedness',
    title: 'RUGGEDNESS',
    description: 'Built for the road, not the showroom. Dust, rain, heat, cold — we\'ve ridden through it all.',
  },
];

export const finalCTA = {
  headline: 'READY TO CHANGE YOUR RIDES FOREVER?',
  cta: 'Pick your Bluarmor Loadout',
};

// About Page Data
export const aboutHero = {
  headline: 'THE HARD WAY IS THE ONLY WAY.',
};

export const aboutStory = [
  {
    id: 'blusnap-origin',
    year: '2017',
    title: 'BluSnap Origin',
    content: 'It started with a problem no one was solving right. Motorcycle communication was fragmented, unreliable, and built for features, not function. We chose to build infrastructure instead.',
  },
  {
    id: 'habit-insight',
    year: '2018',
    title: 'The Habit Insight',
    content: 'We observed riders. We rode with them. We understood that connectivity isn\'t about having more features — it\'s about having reliable ones that disappear into the ride.',
  },
  {
    id: 'infrastructure-pivot',
    year: '2019',
    title: 'The Infrastructure Pivot',
    content: 'We stopped building products and started building systems. The mesh network. The universal protocol. The foundation everything else would stand on.',
  },
  {
    id: 'made-in-india',
    year: '2021',
    title: 'Made in India, For the World',
    content: 'Our engineering happens here. Our manufacturing happens here. Our testing happens on Indian roads — the most demanding proving ground on earth.',
  },
  {
    id: 'instrumentation-team',
    year: '2023',
    title: 'The Instrumentation Team',
    content: 'We built a team of engineers who ride. Every decision is validated by people who will use it. No armchair engineering. No theoretical solutions.',
  },
  {
    id: 'radical-transparency',
    year: '2024',
    title: 'Radical Transparency',
    content: 'We opened our testing data. Our failure rates. Our improvement metrics. Because infrastructure demands accountability.',
  },
  {
    id: 'product-profile',
    year: '2025',
    title: 'Product Profile',
    content: 'Every Bluarmor device now carries a digital identity. Its history. Its updates. Its performance data. Complete traceability.',
  },
  {
    id: 'brand-promise',
    year: '2026',
    title: 'The Brand Promise',
    content: 'We don\'t promise excitement. We promise reliability. We promise that when you need it to work, it will work.',
  },
];

export const timeline = [
  { year: '2017', milestone: 'BluSnap Founded' },
  { year: '2018', milestone: 'First Prototype' },
  { year: '2019', milestone: 'Mesh Technology Patent' },
  { year: '2020', milestone: 'Production Launch' },
  { year: '2021', milestone: 'Manufacturing Scale' },
  { year: '2022', milestone: '50,000 Units' },
  { year: '2023', milestone: 'eRideGrid Launch' },
  { year: '2024', milestone: 'International Expansion' },
  { year: '2025', milestone: 'Product Profile System' },
  { year: '2026', milestone: '2026 Armoury' },
];

// Support Page Data
export const supportHero = {
  headline: 'YOU RIDE. WE RESPOND.',
  subheadline: 'Hardware is hard. The road is harder.',
};

export const supportSections = [
  {
    id: 'product-profile',
    title: 'Product Profile Diagnostics',
    description: 'Every Bluarmor device has a digital identity. Access your product\'s complete history, performance data, and diagnostic information.',
    cta: 'Check Your Device',
  },
  {
    id: 'serviced-here',
    title: 'Built Here. Serviced Here.',
    description: 'Our service network mirrors our manufacturing. Local expertise. Genuine parts. Fast turnaround. No shipping overseas.',
    cta: 'Find Service Centre',
  },
  {
    id: 'knowledge-base',
    title: 'Knowledge Base',
    description: 'Detailed guides, troubleshooting steps, and technical documentation. Written by engineers, for riders.',
    cta: 'Browse Articles',
  },
];

export const supportCTA = 'Access Support Command';

export const faqItems = [
  {
    question: 'How do I update my device firmware?',
    answer: 'Open the Bluarmor app, connect your device, and navigate to Settings > Firmware. Updates are automatic when available.',
  },
  {
    question: 'What is the warranty period?',
    answer: 'All Bluarmor devices come with a 2-year comprehensive warranty covering manufacturing defects and component failures.',
  },
  {
    question: 'How many devices can connect in a mesh?',
    answer: 'The eRideGrid mesh network supports unlimited riders. Each node strengthens the network.',
  },
  {
    question: 'Is the system waterproof?',
    answer: 'All devices are rated IP67 — fully protected against dust and water immersion up to 1 meter for 30 minutes.',
  },
];
